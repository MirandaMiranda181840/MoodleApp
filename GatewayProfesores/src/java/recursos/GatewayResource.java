...
public class GatewayResource {
    ...
    public Response getCalificacionesProfesor(@QueryParam("token")String token, @QueryParam("cursoId") int cursoId, @QueryParam("asignacionId") int asignacionId) {
        DatosAsignacion[] cali;
        try{
            cali = c.getCalificacionesProfesor(DatosAsignacion[].class, asignacionId, cursoId, token);
        }catch(ClientErrorException e){
            return Response.status(500/*, e.getMessage()*/).build();
        }
    ...
    public Response obtenerPadres(@QueryParam("token")String token) {
        DatosPadre[] padres;
        try {
            padres = u.obtenerPadres(DatosPadre[].class, token);
        }catch(ClientErrorException e){
            return Response.status(500/*, e.getMessage()*/).build();
        }
        
        return Response.ok(padres, MediaType.APPLICATION_JSON).build();
    }
